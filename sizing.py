...
# 2. Calculate Expected Returns and Volatilities for Each Pair
pair_metrics = []
...
for _, r in df.iterrows():
    ...
    # Expected profit/loss distances
...
# 5. Convert to Dollar Allocations
metrics_df['kelly_weight'] = optimal_weights
metrics_df['target_capital'] = optimal_weights * acct

final = []

for _, r in metrics_df.iterrows():
    if r['target_capital'] < acct * 0.005:
        continue
    ...
    feas = []
    min_c, max_c = max(acct * 0.005, r['target_capital'] * 0.5), min(acct * 0.15, r['target_capital'] * 1.5)
    ...
    if feas:
        ...
        final.append({
            ...
            'annualized_return': r['annualized_return'], 'kelly_fraction': best['capital']/acct,
            'capital_allocation': best['capital'], 'shares1': best['shares1'], 'shares2': best['shares2'],
...
if final:
    ...
    print(f"Total allocated: ${out['capital_allocation'].sum():,.0f} ({out['capital_allocation'].sum()/acct:.1%})")
    print(f"{'='*60}")
else:
    pd.DataFrame(columns=['stock1','stock2','signal','z_score','hedge_ratio','kelly_fraction',
                          'capital_allocation','shares1','shares2','price1','price2','win_prob',
